...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        $block = $page->getLayout()->getBlock('etechflow.sitemap.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $plan)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_sitemap'))
                  ->setData('management_url', $this->getUrl('etechflow_sitemap/license/gate'));
        }
